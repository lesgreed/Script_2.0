...
def plot_3d(R_x_all, R_y_all, Z_all, P_1, P_2_new):
    ...
    i=0
    while i<=360:
       ax.plot(R_x_all[i], R_y_all[i], Z_all[i], marker='o', linestyle='-', alpha=0.2, markersize=1, linewidth=1)
    ...
    for i in range(len(P_1[0])):
       ax.plot([P_1[0][i], P_2_new[0][i]], [P_1[1][i], P_2_new[1][i]], [P_1[2][i], P_2_new[2][i]],  color='blue', linewidth=1)
    ...
    ax.set_ylabel('Y[cm]')
    ax.set_zlabel('Z[cm]')
    ax.set_title('3D W7-X simplified model')
